...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function runDynamicCmsDomScanner(pageKey) {
        ...
        // Escanear casi todos los elementos textuales de la página (excluyendo controles interactivos del sistema)
        const textElements = rootEl.querySelectorAll('h1, h2, h3, h4, h5, h6, p, span, a, label, strong, li, td, th, caption, .court-title, .court-description, .section-title, .section-subtitle, .section-description, .hero-title, .hero-subtitle');

        let textIdx = 0;
        textElements.forEach(el => {
            // Ignorar elementos críticos del sistema, modales y el propio CMS
            if (el.closest('.modal-overlay') || el.closest('.legal-modal-overlay') || el.closest('#adminCmsFabWrapper') || el.closest('.cms-popover-panel') || el.closest('.nav-actions') || el.closest('.user-badge-nav')) return;
