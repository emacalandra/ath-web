...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function cargarTablaReservas() {
        ...
        const listaARenderizar = window.subtabActualReservas === 'activas' ? reservasActivas : reservasHistorial;
        ...
        bookingsTableBody.innerHTML = listaARenderizar.map(r => {
            ...
            let accionesHtml = '';
            if (subtabActualReservas === 'historial') {
                accionesHtml = `
                ...
                accionesHtml = `
                    <div style="display: flex; flex-direction: column; gap: 8px; min-width: 210px; background: rgba(0, 0, 0, 0.25); padding: 10px; border-radius: 8px; border: 1px solid rgba(255, 255, 255, 0.08);">
                        ...
                        <!-- SECCIÓN B: RECEPCIÓN Y ASISTENCIA -->
                        <div>
                            <span style="font-size: 0.68rem; color: #94A3B8; text-transform: uppercase; letter-spacing: 0.5px; font-weight: 800; display: block; margin-bottom: 6px;">🎾 Recepción (Asistencia):</span>
                            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 6px;">
                                <button class="btn-attend-yes" data-resid="${r.id}" title="Marcar que el jugador llegó al club y asistió" style="background: rgba(59, 130, 246, 0.18); border: 1px solid #3B82F6; color: #60A5FA; padding: 6px 8px; border-radius: 6px; font-size: 0.75rem; font-weight: 700; cursor: pointer; display: inline-flex; align-items: center; justify-content: center; gap: 4px; transition: all 0.2s;">
                                    <i class="fa-solid fa-user-check"></i> Asistió
                                </button>
                                <button class="btn-attend-no" data-resid="${r.id}" title="Marcar que el jugador no se presentó (Ausente)" style="background: rgba(100, 116, 139, 0.18); border: 1px solid #64748B; color: #CBD5E1; padding: 6px 8px; border-radius: 6px; font-size: 0.75rem; font-weight: 700; cursor: pointer; display: inline-flex; align-items: center; justify-content: center; gap: 4px; transition: all 0.2s;">
            ...
            return `
                <tr>
                    ...
                    <td>${canchaTexto}</td>
                    <td>${fechaTexto} &bull; ${horarioTexto}</td>
                    <td>${montoTexto}</td>
                    <td>${metodoPagoHtml}</td>
                    <td><span class="status-badge ${badgeClass}" ${badgeStyle}>${r.estadoPago || '⏳ Pago esperando aprobación'}</span></td>
        ...
        // Escuchadores de Control de Asistencia en Mostrador / Secretaría
        document.querySelectorAll('.btn-attend-yes').forEach(btn => {
            btn.addEventListener('click', async () => {
                const resId = btn.dataset.resid;
                await window.DBHits.actualizarAsistenciaReserva(resId, 'Asistió');
                cargarTablaReservas();
            });
        });
    ...
    async function cargarTablaBloqueosYReservas() {
        ...
        try {
            ...
            adminLocksTableBody.innerHTML = todasReservas.map(r => {
                ...
                return `
                    <tr>
                        <td><strong>Cancha ${r.canchaId}</strong></td>
                        <td>${r.fecha}<br><span style="font-weight:700; color:var(--color-ath-orange);">${r.horaInicio} a ${hFin} hs</span></td>
                        <td>${usuarioDetalle}</td>
                        <td>${badgeTipo}</td>
                        <td>
